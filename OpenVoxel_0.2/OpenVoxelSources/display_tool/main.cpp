...
void usage()
{
  std::cerr 
    ...
    << "sending it the character 'A'/'B'(up/down).  Theimages are then rendered to display, " << std::endl  
...
int main(int argc, char *argv[])
{
  ...
  Widget w(delay_before, delay_after, num_images,&serial_device,&camera_ip,&image_prefix,interlace_levels,numbers,0,format);
  w.resize(608,684);
  if(fullscreen)
    w.showFullScreen();
  else
